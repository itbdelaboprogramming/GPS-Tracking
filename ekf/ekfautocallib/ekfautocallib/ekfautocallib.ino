/** 
  Code to run a rover with 4 modes: 
    ...
    4. Callibration Mode is a mode to run the rover forward for 10s automatically to get an initial heading for EKF computation
  Odometry reading is also added in this code.
...
#define MAX_PWM 30          //PWM Saturation for PID
#define SONAR_NUM 1         //Number of Ultrasonic sensors.
...
void calibMode(){
  if (time_callib < 5000){
    ...
    pwm_ki = pid_left_omega.compute(target_speed_ki, curr_left_angle, MAX_PWM, Ts);
    pwm_ka = pid_right_omega.compute(target_speed_ka, curr_right_angle, MAX_PWM, Ts);
...
void getPose (){
  ...
  // Velocity
  VL = filtered_left_omega*PI/30.0*WHEEL_RADIUS; //in cm/s
  VR = filtered_right_omega*PI/30.0*WHEEL_RADIUS;
}
//----------------------------------------------------------------------------------------------------//
