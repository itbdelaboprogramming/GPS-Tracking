...
% Predicted state
x_prd =[x_est(1, 1) + (dt * Cv * odo_V * sin(x_est(3, 1) - alpha));...            
        x_est(2, 1) + (dt * Cv * odo_V * cos(x_est(3, 1) - alpha));...
        wrapAngle(x_est(3, 1) + (dt * odo_psi_1dot))];

% Jacobian of system function (predicted state)
jac_fx = ([x_est(1, 1)+dx + dt * Cv * odo_V * sin(x_est(3, 1) - alpha),...
          x_est(2, 1) + dt * Cv * odo_V * cos(x_est(3, 1) - alpha),...
          wrapAngle(x_est(3, 1) + dt * odo_psi_1dot);...
          x_est(1, 1) + dt * Cv * odo_V * sin(x_est(3, 1) - alpha),...
          x_est(2, 1)+dx + dt * Cv * odo_V * cos(x_est(3, 1) - alpha),...
          wrapAngle(x_est(3, 1) + dt * odo_psi_1dot);...
          x_est(1, 1) + dt * Cv * odo_V * sin(x_est(3, 1)+dx - alpha),...
          x_est(2, 1) + dt * Cv * odo_V * cos(x_est(3, 1)+dx - alpha),...
