...
void loop() {
  ...
  if (curr_millis - prev_millis >= 10) {
    ...
    int pwm_ki = pid_left_omega.compute(target_omega,filtered_left_omega,max_pwm);
    int pwm_ka = pid_right_omega.compute(target_omega,filtered_right_omega,max_pwm);
...
pidIr pid_left_omega(1.4216825,2.775639398672394,0.0910232220625,LOOPTIME/1000.0); //(1.3682225,2.671266106989457,0.0876004455625)
pidIr pid_right_omega(1.3682225,2.671266106989457,0.0876004455625,LOOPTIME/1000.0);//(1.4216825,2.775639398672394,0.0910232220625)

//Kp = 0.5, Ki = 2.0, Kd = 0.016; (0.5,2.0,0.016)
//(0.0,0.0,0.0)
// Create LPF object
float fc = 3; //fc = cut-off frequency (in Hz)
float Ts = LOOPTIME/1000.0; //Ts = time sampling (in s)
...
void loop() {
  ...
  if (curr_millis - prev_millis >= 10) {
    ...
    if(ch3 <= 1000){
      ...
      if(ch1 >= 1500+SIGNAL_THRESHOLD){
        moveValue = map(ch1, 1500+SIGNAL_THRESHOLD, 2000, 0, MAX_RPM_MOVE);
      } else if(ch1 <= 1500-SIGNAL_THRESHOLD){
        moveValue = map(ch1, 1500-SIGNAL_THRESHOLD, 1000, 0, -MAX_RPM_MOVE);
      } else {
        moveValue = 0;
      }

      if(ch2 >= 1500+SIGNAL_THRESHOLD){
        turnValue = map(ch2, 1500+SIGNAL_THRESHOLD, 2000, 0, MAX_RPM_TURN);
      } else if(ch2 <= 1500-SIGNAL_THRESHOLD){
        turnValue = map(ch2, 1500-SIGNAL_THRESHOLD, 1000, 0, -MAX_RPM_TURN);
      } else {
        turnValue = 0;
      }
      
      target_omega = moveValue;
