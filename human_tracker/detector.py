...
# check_horizontal_direction: used to tell direction of an object for a fixed camera position
def check_horizontal_direction_fixed(prevx, curx):
    if abs(curx-prevx) <= 15:
        return "idle"
    elif curx-prevx < -15:
        return "left"
    else:
...
def get_centered_contours(mask):
    ...
    if sorted_contours != []:
        ...
        for k in range(len(sorted_contours)):
            if cv2.contourArea(sorted_contours[k]) < 500.0:
...
def check_colour(roi):
    if 0 in np.shape(roi):
        ...
        lower_hsv = np.array([36, 117, 182])
        upper_hsv = np.array([61, 224, 255])
